...
public class TestServiceLocator : IServiceLocator
{
    ...
    public object GetRequiredService(Type serviceType)
    {
            ...
            serviceType == typeof(IRequestHandler<CreateTaskCommand, TaskDto>))
        {
            ...
            return new CreateTaskCommandHandler(taskCommandRepository, userQueryRepository, _context, mockLogger.Object,
                mockAuditLogService.Object, mockTaskHistoryService.Object);
        }
            ...
            serviceType == typeof(IRequestHandler<AcceptTaskCommand, TaskDto>))
        {
            ...
            var mockTaskHistoryService = new Mock<ITaskHistoryService>();
            return new AcceptTaskCommandHandler(currentDateService, taskCommandRepository, userQueryRepository,
                _context, mockTaskHistoryService.Object);
        }
        ...
        {
            var currentDateService = GetService<ICurrentDateService>();
            return new RejectTaskCommandHandler(currentDateService, taskCommandRepository, userQueryRepository,
                _context);
        }
        ...
        // Handle services
        if (serviceType == typeof(IReminderCalculationService)) return reminderCalculationService;
        ...
        if (serviceType.IsGenericType && serviceType.GetGenericTypeDefinition() == typeof(IValidator<>))
        {
            ...
            if (requestType == typeof(CreateTaskCommand)) return new CreateTaskCommandValidator();

            if (requestType == typeof(AssignTaskCommand)) return new AssignTaskCommandValidator();

            if (requestType == typeof(UpdateTaskProgressCommand)) return new UpdateTaskProgressCommandValidator();
    ...
    public IEnumerable<object> GetServices(Type serviceType)
    {
        ...
        {
            var requestType = serviceType.GetGenericArguments()[0];
            if (requestType == typeof(CreateTaskCommand)) return new List<object> { new CreateTaskCommandValidator() };

            if (requestType == typeof(AssignTaskCommand)) return new List<object> { new AssignTaskCommandValidator() };
