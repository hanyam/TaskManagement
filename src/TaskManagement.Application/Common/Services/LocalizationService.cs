...
public class LocalizationService(IUserSettingsService userSettingsService) : ILocalizationService
{
    ...
    public string GetString(string key, string? defaultValue = null, params object[] args)
    {
        ...
        if (language != "en")
        {
            if (!_resourceCache.ContainsKey("Resources.en.json"))
            {
                LoadResources("en");
            }

            if (_resourceCache.TryGetValue("Resources.en.json", out var enResources) && enResources.TryGetValue(key, out var enValue))
            {
                // Format with arguments if provided
