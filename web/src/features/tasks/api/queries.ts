...
export function useCreateTaskMutation() {
  ...
  return useMutation(async (payload: CreateTaskRequest) => {
    const { data } = await apiClient.request<TaskDto>({
      path: "/tasks",
      method: "POST",
      body: payload,
    ...
    await queryClient.invalidateQueries(taskKeys.lists());
    return data;
  });
}
...
export function useAssignTaskMutation(taskId: string) {
  ...
  return useMutation(async (payload: AssignTaskRequest) => {
    const { data } = await apiClient.request<TaskDto>({
    ...
    await queryClient.invalidateQueries(taskKeys.detail(taskId));
    await queryClient.invalidateQueries(taskKeys.lists());
    return data;
  });
}
...
export function useUpdateTaskProgressMutation(taskId: string) {
  ...
  return useMutation(async (payload: UpdateTaskProgressRequest) => {
...
export function useAcceptTaskProgressMutation(taskId: string) {
  ...
  return useMutation(
    async (payload: { progressHistoryId: string }) => {
...
export function useRejectTaskMutation(taskId: string) {
  const locale = useCurrentLocale();
  const queryClient = useQueryClient();
  return useMutation(async (payload: { reason?: string | null }) => {
    const { data } = await apiClient.request<TaskDto>({
...
export function useRequestMoreInfoMutation(taskId: string) {
  const locale = useCurrentLocale();
  const queryClient = useQueryClient();
  return useMutation(async (payload: RequestMoreInfoRequest) => {
    const { data } = await apiClient.request<TaskDto>({
...
export function useRequestExtensionMutation(taskId: string) {
  const locale = useCurrentLocale();
  const queryClient = useQueryClient();
  return useMutation(async (payload: RequestDeadlineExtensionRequest) => {
...
export function useApproveExtensionRequestMutation(taskId: string) {
  ...
  const queryClient = useQueryClient();
  return useMutation(
    async ({ requestId, reviewNotes }: { requestId: string; reviewNotes?: string | null }) => {
      await apiClient.request<void>({
